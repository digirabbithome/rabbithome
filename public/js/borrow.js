...
import { collection, addDoc, serverTimestamp, query, orderBy, where, doc, updateDoc, onSnapshot, arrayUnion, getDoc } from 'https://www.gstatic.com/firebasejs/11.10.0/firebase-firestore.js'
...
function bindUI(){
  ...
  document.getElementById('kw').addEventListener('input', e=>{ kw = e.target.value.trim(); updatePage() })
...
function previewFiles(e){
  ...
  files.slice(0,12).forEach(f=>{ const url = URL.createObjectURL(f); const img = document.createElement('img'); img.src = url; box.appendChild(img) })
}
...
async function onSubmit(ev){
  ...
  if(files.length){
    const prog = document.getElementById('upProgress'); let done = 0
    for(const f of files){
      ...
      await new Promise((resolve,reject)=>{
        ...
        task.on('state_changed', null, reject, async ()=>{
          ...
          done++; if(prog) prog.textContent = `上傳 ${done}/${files.length}`; resolve()
        })
      })
    }
  ...
  ev.target.reset(); document.getElementById('photoPreview').innerHTML=''
}
...
function attachRealtime(){
  ...
  unsub = onSnapshot(q, snap => {
    ...
    if(kw){
      ...
      rows = rows.filter(r=> (r.itemName||'').toLowerCase().includes(K) || (r.assignee||'').toLowerCase().includes(K) || (r.desc||'').toLowerCase().includes(K))
    }
    window._rows = rows; window._page = 1; updatePage()
  }, err => { console.error('onSnapshot error', err); const warn = document.getElementById('ruleWarn'); if (warn) warn.style.display = 'inline' })
}
function updatePage(){ const rows=(window._rows||[]); const page=window._page||1; const start=(page-1)*pageSize; const end=start+pageSize; renderTable(rows.slice(start,end))
  document.getElementById('pageInfo').textContent = `第 ${page} 頁 / 共 ${Math.max(1, Math.ceil(rows.length/pageSize))} 頁（${rows.length} 筆）`
  document.getElementById('prevPage').disabled = page<=1; document.getElementById('nextPage').disabled = end>=rows.length }
function paginate(dir){ const rows=window._rows||[]; const total=Math.max(1, Math.ceil(rows.length/pageSize)); window._page=Math.min(total, Math.max(1, (window._page||1)+dir)); updatePage() }
function badgeHTML(s){ const map={ pending:'未完成', returned:'已還回', refunded:'已退款' }; return `<span class="badge ${s}">${map[s]||s}</span>` }
function renderTable(rows){
  ...
  for(const r of rows){
    ...
    const u=(r.photos && r.photos[0]?.url) || ''
    const img = u ? `<div class="thumb-wrap" title="點擊開新分頁檢視原圖">
      <a class="thumb-link" href="${u}" target="_blank" rel="noopener noreferrer">
        <img src="${u}" class="thumb" alt="thumb">
        <img src="${u}" class="thumb-large" alt="preview" style="display:none">
      </a></div>` : ''
    ...
    tr.innerHTML = `<td>${time}</td>
      ...
      <td class="row-actions">${ r.status==='pending' ? `<button class="btn" data-act="returned" data-id="${r.id}">✅ 還回</button><button class="btn" data-act="refunded" data-id="${r.id}">💸 退款</button>` : '' }${ r.status!=='pending' ? `<button class="btn" data-act="reopen" data-id="${r.id}">↩️ 重新開啟</button>` : '' }</td>`
    tb.appendChild(tr)
  }
  tb.querySelectorAll('button[data-id]').forEach(btn=>{ btn.addEventListener('click', async ()=>{
    const id=btn.dataset.id, act=btn.dataset.act
    if(act==='returned' || act==='refunded'){ await updateDoc(doc(db,'borrows',id), { status: act, updatedAt: serverTimestamp() }) }
    else if(act==='reopen'){ await updateDoc(doc(db,'borrows',id), { status: 'pending', updatedAt: serverTimestamp() }) }
  })})
}
