...
async function loadProfile() {
  ...
  profile = snap.data() || {}
  document.getElementById('profile').innerHTML = `${profile.nickname || profile.name || me.email}`
  renderQuota()
}
...
function bindForm(){
  document.getElementById('submit').onclick = async () => {
    ...
    if(end<start) return alert('結束日期不得早於開始日期')
...
async function loadLeaves(){
  ...
  if (snap.empty) { list.innerHTML = '<div class="row">尚無申請</div>'; return }

  // 同步算當下「總額」，讓每一筆旁邊能顯示 x/總額
  const p = profile.annualLeavePolicy || {}
  const eff = p.effectiveFrom ? new Date(p.effectiveFrom+'T00:00:00') : null
  const now = new Date()
  const quota = (eff && now<eff) ? 0 : (Number(p.baseDays||0) + Number(p.carryOverDays||0) + Number(p.manualAdjust||0))
  ...
  snap.forEach(d=>{
    ...
    rows.push(`<div class="row">
      ...
      <span>${l.days || '-'} / ${quota}</span>
      <span><span class="badge ${l.status}">${statusCN}</span></span>
    </div>`)
  })
