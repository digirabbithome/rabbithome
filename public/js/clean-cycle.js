// clean-cycle-fb.js — 暱稱從 /users/{uid}.nickname，資料寫入 Firestore
...
let me = null, myNickname = '', tasks = [], currentFilter='all', editingId=null, chart=null, historyCache=[]
...
window.onload = () => {
  onAuthStateChanged(auth, async (user)=>{
    ...
    me = user; myNickname = await resolveNickname(user.uid);
...
function getStatus(task){
  const cycle = clampInt(task.days,1,3650); const dueAt = addDays(task.last||nowIso(), cycle);
  const d = daysBetween(nowIso(), dueAt); let status='ok'; if (d<=2 && d>0) status='soon'; if (d<=0) status=(d===0)?'due':'over';
...
function matchFilter(status, task){
  ...
  if (currentFilter==='done-today'){ const todayStr=(new Date()).toDateString(); return new Date(task.last).toDateString()===todayStr }
  return true
}
...
function rowEl({head=false, task=null, st=null}){
  ...
  if(head){ row.innerHTML=`<div>區域</div><div>項目</div><div>週期(天)</div><div>下次到期</div><div>狀態</div><div>上次完成 / 備註 / 操作</div>`; div.appendChild(row); return div; }
  ...
  row.querySelector('[data-act="del"]').addEventListener('click', ()=> removeTaskConfirm(task.id));
...
function renderList(){
  ...
  let due=0, over=0, doneToday=0; const todayStr=(new Date()).toDateString();
  tasks.forEach(task=>{ const st=getStatus(task); if(!matchFilter(st.status, task)) return;
    if(st.status==='due') due++; if(st.status==='over') over++; if(new Date(task.last).toDateString()===todayStr) doneToday++;
...
async function completeOne(id){
  const t=tasks.find(x=>x.id===id); if(!t) return; const last=nowIso();
  await updateTask(id, { last }); await pushHistory({ taskId:id, area:t.area, name:t.name, days:t.days, note:t.note||'', doneBy: myNickname, doneByUid: me?.uid||null });
}
async function resetLast(id){ if(!confirm('要把「上次完成」重設為現在嗎？')) return; await updateTask(id, { last: nowIso() }); }
async function removeTaskConfirm(id){ const t=tasks.find(x=>x.id===id); if(!t) return; if(!confirm(`確定刪除「${t.area}-${t.name}」？`)) return; await removeTask(id); }

async function completeAllDue(){
  let changed=0; for (const t of tasks){ const st=getStatus(t); if(st.status==='due'||st.status==='over'){ await updateTask(t.id,{last:nowIso()}); await pushHistory({ taskId:t.id, area:t.area, name:t.name, days:t.days, note:t.note||'', doneBy: myNickname, doneByUid: me?.uid||null, action:'bulk-complete' }); changed++; } }
  if(!changed) alert('目前沒有到期/逾期項目。')
}

function openAddDialog(){ editingId=null; document.getElementById('dlgTitle').textContent='新增項目';
  document.getElementById('fArea').value=''; document.getElementById('fName').value=''; document.getElementById('fDays').value=7; document.getElementById('fLast').value=''; document.getElementById('fNote').value=''; document.getElementById('taskDlg').showModal(); }
function openEditDialog(id){ const t=tasks.find(x=>x.id===id); if(!t) return; editingId=id; document.getElementById('dlgTitle').textContent='編輯項目';
  document.getElementById('fArea').value=t.area||''; document.getElementById('fName').value=t.name||''; document.getElementById('fDays').value=clampInt(t.days,1,3650); const dt=new Date(t.last||nowIso()); document.getElementById('fLast').value=dt.toISOString().slice(0,16); document.getElementById('fNote').value=t.note||''; document.getElementById('taskDlg').showModal(); }
...
function renderContribChart(){
  ...
  const labels=Object.keys(counts); const values=labels.map(k=>counts[k]); const sum=values.reduce((a,b)=>a+b,0)||1; const percents=values.map(v=>Math.round(v/sum*1000)/10);
  if(chart){ chart.destroy(); }
  chart=new Chart(ctx,{ type:'bar', data:{ labels, datasets:[{ label:'完成次數占比(%)', data:percents, borderWidth:1 }] }, options:{ indexAxis:'y', responsive:true, plugins:{ legend:{display:false}, tooltip:{ callbacks:{ label:(c)=>`${c.raw}%（${values[c.dataIndex]} 次）` } } }, scales:{ x:{ min:0,max:100, ticks:{ callback:(v)=> v+'%' } } } } });
}
