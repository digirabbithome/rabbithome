...
window.addEventListener('load', async () => {
  ...
  // 初始化自訂寄件公司顯示
  if (otherField) otherField.style.display = companySelect && companySelect.value === '其他' ? 'block' : 'none';
  ...
  async function handleSubmit(type = "normal") {
    ...
    const checkedSources = Array.from(form.querySelectorAll('input[name="source"]:checked')).map(n => n.value.trim()).filter(Boolean);
    const sourceStr = checkedSources.join('、');
    const nickname = localStorage.getItem('nickname') || '匿名';

    const fullSource = sourceStr ? `${nickname}(${sourceStr})` : nickname;
    const displaySource = type === "reply"
      ? (sourceStr ? `${nickname}(${source})(回郵)` : `${nickname}(回郵)`)
      : (sourceStr ? `${nickname}(${source})` : nickname);
