...
function statusZh(s){ return s==='pending'?'待審核': s==='approved'?'已核准': s==='rejected'?'已拒絕': s }

function computeQuotaLeft(u){
...
async function refresh(){
  ...
  for(const d of allSnap.docs){
    ...
    const {quota, left} = computeQuotaLeft(u)
    const row=document.createElement('div'); row.className='row'; row.innerHTML=`
      ...
      <span><span class="badge ${l.status}">${statusZh(l.status)}</span></span>
  ...
  list.onclick=async e=>{
    ...
    if(act==='approve'){
      const uSnap=await getDoc(doc(db,'users',uid)); const u=uSnap.exists()? uSnap.data(): {}
      const dates=expandDates(l.start,l.end); const need=dates.length
      const {left} = computeQuotaLeft(u)
      const allowOverride = document.getElementById('allowOverride').checked
      if (l.type==='annual' && need>left && !allowOverride){
        alert(`年假剩餘不足：需求 ${need} 天，剩餘 ${left} 天。
請調整配額或勾選「允許剩餘不足仍核准」。`)
        return
      }
      ...
      // 若是年假且足夠或允許超扣：累計已用（允許超扣則可能變成負剩餘）
      if (l.type==='annual'){
        const year=new Date().getFullYear()
        const cur = (u.leaveStats && u.leaveStats.year===year) ? (u.leaveStats.approvedTaken||0) : 0
        batch.update(doc(db,'users',uid), { leaveStats: { year, approvedTaken: Number(cur)+need } })
      }
